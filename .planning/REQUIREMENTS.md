...
- [x] **INGEST-05**: System stores extraction metadata (method, quality score, page strategies, library version) with each document in `text_extraction_meta` JSONB column
...
- [x] **STORE-04**: System migrates existing manual `health_entries` records into new patient tables without data loss
...
- [x] **RETRIEVE-04**: System classifies query intent and routes numeric/aggregation queries to Supabase SQL, narrative queries to Azure AI Search
- [x] **RETRIEVE-05**: Context injected into Claude uses structured entity blocks (lab values, flags, summaries) not 500-char raw text truncations
